...
public sealed class MinMax : MinMaxGeneric<double>, ICloneable
{
    ...
    public static List<MinMax> Cut(IEnumerable<MinMax> src, IEnumerable<MinMax>? cutters)
    {
        ...
        return src as List<MinMax> ?? src.ToList();
    }
    ...
    {
        if (append is null) return src.ToList();
    ...
    {
        if (range is null) return new List<MinMax>();
    ...
    public static MinMax operator /(MinMax? a, double b)
    {
        ...
        if (a is null || a.IsZeroOnInvalid || b == 0) return new MinMax();
    ...
    public static MinMax operator *(MinMax? a, MinMax? b)
    {
        if (a is null || a.IsZeroOnInvalid || b is null || b.IsZeroOnInvalid) return new MinMax();
    ...
    public static MinMax? operator -(MinMax? a, double b)
    {
        return a is null ? null : new MinMax(a.Min - b, a.Max - b);
    }

    public static MinMax? operator -(MinMax? a) { return a is null ? null : new MinMax(-a.Max, -a.Min); }
